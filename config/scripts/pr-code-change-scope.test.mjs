...
describe('PR Checks skip wiring', () => {
  ...
  it('keeps the cheap root-directory guard on docs-only PRs', () => {
    ...
    expect(prWorkflow.jobs.formatting.if).toBeUndefined()
    expect(prWorkflow.jobs.formatting.needs).toBeUndefined()
  })
  ...
  it('lets verify pass skipped jobs the classifier turned off', () => {
    ...
    expect(verifyStep.run).toContain('"$FORMATTING" != "success"')
    ...
    for (const job of prWorkflow.jobs.verify.needs) {
      if (job === 'code_paths' || job === 'root_directory_guard' || job === 'formatting') {
        continue
      }
